...
export function AsciiLogo({
  ...
  useEffect(() => {
    ...
    const setPhase = (next: AsciiLogoPhase) => {
      ...
      phase = next
      propsRef.current.onPhaseChange?.(next)
    }
    ...
    const rebuild = async () => {
      ...
      const snapshot = {
        ...propsRef.current,
        src: image ? propsRef.current.src : undefined,
      }
    ...
    const tick = (now: number) => {
      ...
      update(reduceRef.current ? 0 : frames)
      draw()
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      const step = Math.max(
        4,
        propsRef.current.cellSize + propsRef.current.cellGap
      )
