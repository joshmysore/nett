...
description: "Parked Elaya redesign skill from github.com/elayadesign/redesign-skill. Use only when the user explicitly asks to audit or upgrade an existing site with this skill. Do not apply to Nett workbench routes or routine UI. Diagnoses generic AI patterns and applies in-place fixes."
...
Installed from [elayadesign/redesign-skill](https://github.com/elayadesign/redesign-skill) for a later landing-page pass. Until the user explicitly invokes this skill, ignore it.

When invoked:

- Report the diagnosis first. Do not edit until the user approves.
- Scope is `/` and `/about` unless the user names another surface.
- `design.md`, `AGENTS.md`, and `nett-visual-direction` override this file’s Design Values. Remap onto Nett tokens; do not replace the locked system.
- Do not restyle Ask, Review, People, or Sources.
- Companion for greenfield landing work: `landing-page-design`. Hallmark remains the anti-slop companion.
