...
description: "Parked Elaya landing-page skill from github.com/elayadesign/ai-design-skills. Use only when the user explicitly asks to apply landing-page-design or Elaya landing skills. Do not apply to Nett workbench routes or routine UI. Builds a landing page from scratch: intake, structure, conversion copy, and a visual system."
disable-model-invocation: true
---

# Landing Page Design

## Nett (parked — do not apply yet)

Installed from [elayadesign/ai-design-skills](https://github.com/elayadesign/ai-design-skills) for a later landing-page pass. Until the user explicitly invokes this skill, ignore it.

When invoked:

- Scope is `/` and `/about` unless the user names another surface.
- `design.md`, `AGENTS.md`, and `nett-visual-direction` override this file’s Design Values (Tailwind type scale, Phosphor / Solar / Iconamoon, hex dark palette, floating island nav, hero text gradients). Remap onto Nett tokens; do not replace the locked system.
- Do not restyle Ask, Review, People, or Sources.
- Hallmark remains the anti-slop companion. This skill is conversion, structure, and Elaya visual rules — remapped onto Nett.
...
On Nett, the parked preamble above wins: landing `/` and `/about` only, and `design.md` overrides this file’s Design Values. Elsewhere, apply to landing-page work. When a rule here conflicts with a framework default, this file wins unless a project design system (here, `design.md`) is locked. When the user's explicit prompt conflicts with a rule, the user wins.
