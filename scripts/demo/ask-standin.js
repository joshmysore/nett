...
(() => {
  ...
  window.fetch = async (input, init) => {
    ...
    const stages = brief
      ? [
          { wait: 520, event: { type: "stage", id: "search", label: "Searching records" } },
          { wait: 900, event: { type: "stage", id: "match", label: "Matched 1 person", detail: "local-evidence · Kendra Mysore" } },
          { wait: 780, event: { type: "stage", id: "messages", label: "Reading Messages", detail: "September visit · Dad in town" } },
          { wait: 700, event: { type: "stage", id: "whatsapp", label: "Reading WhatsApp", detail: "Train times" } },
          { wait: 640, event: { type: "stage", id: "write", label: "Writing with llama3.2:3b" } },
        ]
      : [
          { wait: 500, event: { type: "stage", id: "search", label: "Searching records" } },
          { wait: 980, event: { type: "stage", id: "match", label: "Matched 4 people", detail: "Messages · WhatsApp · notes" } },
          { wait: 880, event: { type: "stage", id: "messages", label: "Reading Messages", detail: "Ada Fong · Jordan Lee" } },
          { wait: 840, event: { type: "stage", id: "whatsapp", label: "Reading WhatsApp", detail: "Noor Alvi · Dana Ruiz" } },
          { wait: 1100, event: { type: "stage", id: "relate", label: "Connecting overlapping conversations", detail: "court records · civic data · legal ops" } },
          { wait: 680, event: { type: "stage", id: "write", label: "Writing with llama3.2:3b" } },
        ];

    const words = result.answer.split(/(\s+)/);
    const tokens = [];
    for (let i = 0; i < words.length; i += 5) tokens.push(words.slice(i, i + 5).join(""));
